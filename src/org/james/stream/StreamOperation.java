...
public class StreamOperation {
	...
	public static void testSorted3() {
		Student[] arr1 = { new Student("a", 19d), new Student("a", 13d), new Student("a", 6d), new Student("b", 88d),
				new Student("b", 66d) };
		Arrays.stream(arr1).sorted(Comparator.comparing(Student::getName).thenComparing(Student::getScore))
				.forEach(System.out::println);
	}
	...
	public static void testResult() {
	    ...
	    Map<String,Double> map = Arrays.stream(students).collect(Collectors.toMap(Student::getName,Student::getScore,(s,a)->s+a));
	    ...
	    //统计
		DoubleSummaryStatistics summaryStatistics = Arrays.stream(students).collect(Collectors.summarizingDouble(Student::getScore));
	...
	public static void testGroupAndPiece() {
		...
		Map<Boolean, List<Student>> map1 = Arrays.stream(students).collect(Collectors.partitioningBy(x -> x.getScore() > 50));
		map1.forEach((x, y) -> System.out.println(x + "->" + y));
		
		//downstream指定类型
		Map<String,Set<Student>> map2 = Arrays.stream(students).collect(Collectors.groupingBy(Student::getName,Collectors.toSet()));
	     ...
	     * counting
	     */
	    Map<String,Long> map11 = Arrays.stream(students).collect(Collectors.groupingBy(Student::getName,Collectors.counting()));
	    map11.forEach((x,y)-> System.out.println(x+"->"+y));
	    /**
	     * summingInt
	     */
	    System.out.println("-----------------");
	    Map<String,Double> map12 = Arrays.stream(students).collect(Collectors.groupingBy(Student::getName,Collectors.summingDouble(Student::getScore)));
	    map12.forEach((x,y)-> System.out.println(x+"->"+y));
	    /**
	     * maxBy
	     */
	    System.out.println("=================");
	    Map<String,Optional<Student>> map13 = Arrays.stream(students).collect(Collectors.groupingBy(Student::getName,Collectors.maxBy(Comparator.comparing(Student::getScore))));
	    map13.forEach((x,y)-> System.out.println(x+"->"+y));
	    /**
	     * mapping
	     */
	    Map<String,Set<Double>> map14 = Arrays.stream(students).collect(Collectors.groupingBy(Student::getName,Collectors.mapping(Student::getScore,Collectors.toSet())));
    ...
    public static void testPeek() {
        ...
        stream.peek(StreamOperation::peek1).filter(x -> x > 5)
                .peek(StreamOperation::peek2).filter(x -> x < 8)
                .peek(StreamOperation::peek3)
                .forEach(System.out::println);
    }
